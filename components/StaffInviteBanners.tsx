    ...
    <div className="space-y-3 mb-8">
      {invites.map((invite) => {
        ...
        return (
          <div
            key={invite.id}
            className="p-5 bg-[#F4F1EA]/10 border border-[#F4F1EA]/20 rounded-xl"
          >
            <div className="flex items-start gap-3 mb-4">
              <div className="w-9 h-9 rounded-full bg-[#F4F1EA]/10 flex items-center justify-center shrink-0">
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={isBusy}
                className="flex-1 py-2.5 bg-[#F4F1EA] text-[#0E0E12] font-semibold rounded-lg hover:bg-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isBusy ? '…' : 'Accept'}
              </button>
              <button
