...
/**
 * Open Door signup — no invite codes, no role fork, mirrors the app.
 * New Spaces start unlisted; publishing to Discover goes through the
 * admin review queue. /join (the old invite-gated flow) redirects here.
 */

type Step = 'account' | 'welcome'
...
export default function SignupPage() {
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      // Save username to localStorage - will be applied after email verification
      // (User isn't authenticated until they verify email, so we can't update profile yet)
  ...
  return (
    <div className="min-h-screen bg-[#0E0E12] flex flex-col items-center justify-center px-4 py-12">
      ...
      <div className="flex items-center gap-3 mb-12">
        {['account', 'welcome'].map((s, i) => (
          <div key={s} className="flex items-center">
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold transition-colors ${
                step === s
                  ? 'bg-[#B76E79] text-[#F4F1EA]'
                  : ['account', 'welcome'].indexOf(step) > i
        ...
        {step === 'account' && (
          <div className="space-y-8">
            <div className="text-center">
              <h1 className="text-3xl md:text-4xl font-bold mb-3 text-[#F4F1EA]">
                Start your space
              </h1>
              <p className="text-[#F4F1EA]/60">
                Free to start — no invite needed
              </p>
            </div>
            ...
            <div className="text-center text-[#F4F1EA]/40 text-sm">
              <p>
        ...
        {step === 'welcome' && (
          <div className="space-y-8">
            ...
            <div className="bg-[#1A1A20] rounded-xl p-5 border border-[#2A2A30]">
              <p className="text-sm text-[#F4F1EA]/60 mb-2">Your space link</p>
            ...
            {/* Download App */}
            <div className="bg-[#1A1A20] rounded-xl p-5 border border-[#2A2A30]">
              <p className="text-sm text-[#F4F1EA]/60 mb-3">Download the app to start building your space</p>
              <a
            ...
            <div className="bg-[#1A1A20] rounded-xl p-5 border border-[#2A2A30]">
              <p className="text-sm font-medium mb-4 text-[#F4F1EA]">Quick start checklist</p>
